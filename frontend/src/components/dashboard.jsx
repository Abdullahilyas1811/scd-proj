...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (token) {
      fetch("http://localhost:5000/auth/user-groups", {
        method: "GET",
        headers: {
  ...
  const handleOpen = async (group) => {
    ...
    if (token) {
      try {
        const response = await fetch(`http://localhost:5000/api/capsules?group=${group._id}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const saveRoles = async () => {
    if (token && currentGroup) {
      ...
      try {
        const response = await fetch(`http://localhost:5000/auth/groups/${currentGroup._id}/roles`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
